...
import { 
  ...
  User, Book, MessageSquare, Zap, LogIn, Star, CheckCircle, Target
} from 'lucide-react';
...
export const AdminActivityFeed = () => {
  ...
  const loadChartData = async () => {
    ...
    try {
      ...
      const { data: allActivity } = await supabase
        ...
        .in('action_type', ['book_created', 'book_read', 'plan_completed', 'book_completed', 'summary_generated'])
        .gte('created_at', start.toISOString())
        .lte('created_at', end.toISOString());

      const data: ActivityChartData[] = days.map(day => {
        const dayStart = new Date(day);
        const dayEnd = new Date(day);
        dayEnd.setHours(23, 59, 59, 999);

        const dayItems = allActivity?.filter(a => {
        ...
        return {
          ...
          books_created: dayItems.filter(a => a.action_type === 'book_created').length,
          books_read: dayItems.filter(a => a.action_type === 'book_read' || a.action_type === 'book_completed').length,
          plans_completed: dayItems.filter(a => a.action_type === 'plan_completed').length,
          summaries: dayItems.filter(a => a.action_type === 'summary_generated').length,
        };
      });
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      ...
      case 'book_completed': return <CheckCircle className="w-4 h-4" />;
      case 'book_created': return <Book className="w-4 h-4" />;
      case 'plan_completed': return <Target className="w-4 h-4" />;
  ...
  const getActivityColor = (type: string) => {
    switch (type) {
      ...
      case 'book_created': return 'bg-blue-500/10 text-blue-600 border-blue-500/20';
      case 'plan_completed': return 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20';
      ...
      <Card className="glass-morphism border-primary/20">
        <CardHeader className="pb-4">
          <CardTitle className="flex items-center gap-2 text-lg">
            ...
            User Interactions (Last 14 Days)
          </CardTitle>
          <CardDescription>
            Books created, read, plans completed & summaries generated
          </CardDescription>
        </CardHeader>
        <CardContent>
          {chartLoading ? (
            ...
            <div className="h-[280px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  />
                  <Legend />
                  <Bar dataKey="books_created" name="Books Created" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="books_read" name="Books Read" fill="hsl(var(--accent))" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="plans_completed" name="Plans Done" fill="hsl(var(--secondary))" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="summaries" name="Summaries" stackId="extra" fill="hsl(var(--muted-foreground))" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
            ...
            <div className="flex items-center gap-2">
              <Select value={filterType} onValueChange={setFilterType}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="book_created">Created</SelectItem>
                  <SelectItem value="plan_completed">Plans Done</SelectItem>
